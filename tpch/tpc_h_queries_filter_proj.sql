...
/*ssd query: Q1*/
select l_returnflag, l_linestatus,l_quantity, l_extendedprice, l_discount, l_tax from LINEITEM where l_shipdate <= '1998-08-15';
/******************/
...
/* ssd Query: Q3 */
select l_orderkey, l_extendedprice, l_discount, o_orderdate, o_shippriority from CUSTOMER, ORDERS, LINEITEM where c_mktsegment = 'AUTOMOBILE' and c_custkey = o_custkey and l_orderkey = o_orderkey and o_orderdate < '1995-03-13' and l_shipdate > '1995-03-13';
/*****************/
...
/* ssd Query: Q4 */
select o_orderpriority from ORDERS where o_orderdate >= '1995-01-01' and o_orderdate < '1995-04-01' and exists (select*from LINEITEM where l_orderkey = o_orderkey and l_commitdate < l_receiptdate);
/*****************/
...
/* ssd Query: Q5 */
select n_name, l_extendedprice, l_discount from CUSTOMER, ORDERS, LINEITEM, SUPPLIER, NATION, REGION where c_custkey = o_custkey and l_orderkey = o_orderkey and l_suppkey = s_suppkey and c_nationkey = s_nationkey and s_nationkey = n_nationkey and n_regionkey = r_regionkey and r_name = 'MIDDLE EAST' and o_orderdate >= '1994-01-01' and o_orderdate < '1995-01-01';
...
/* ssd Query: Q7 */
select n1.n_name as supp_nation, n2.n_name as cust_nation, strftime('%Y', l_shipdate) as l_year, l_extendedprice*(1 - l_discount) as volume from SUPPLIER, LINEITEM, ORDERS, CUSTOMER, NATION n1, NATION n2 where s_suppkey = l_suppkey and o_orderkey = l_orderkey and c_custkey = o_custkey and s_nationkey = n1.n_nationkey and c_nationkey = n2.n_nationkey and ((n1.n_name = 'JAPAN' and n2.n_name = 'INDIA') or (n1.n_name = 'INDIA' and n2.n_name = 'JAPAN')) and l_shipdate between '1995-01-01' and '1996-12-31';
/*****************/
...
/* ssd Query: Q8 */
select strftime('%Y', o_orderdate) as o_year, l_extendedprice*(1 - l_discount) as volume, n2.n_name as nation from PART, SUPPLIER, LINEITEM, ORDERS, CUSTOMER, NATION n1, NATION n2, REGION where p_partkey = l_partkey and s_suppkey = l_suppkey and l_orderkey = o_orderkey and o_custkey = c_custkey and c_nationkey = n1.n_nationkey and n1.n_regionkey = r_regionkey and r_name = 'ASIA'	and s_nationkey = n2.n_nationkey and o_orderdate between '1995-01-01' and '1996-12-31'and p_type = 'SMALL PLATED COPPER';
/*****************/
...
/* Host Query: Q9 */
select nation, o_year, sum(amount) as sum_profit from TABLE1 group by nation, o_year order by nation, o_year desc;

/* ssd Query: Q9 */
select n_name as nation, strftime('%Y', o_orderdate) as o_year, l_extendedprice*(1 - l_discount) - ps_supplycost*l_quantity as amount from PART, SUPPLIER, LINEITEM, PARTSUPP, ORDERS, NATION where s_suppkey = l_suppkey and ps_suppkey = l_suppkey and ps_partkey = l_partkey and p_partkey = l_partkey and o_orderkey = l_orderkey and s_nationkey = n_nationkey and p_name like '%dim%';
/*****************/
...
/* ssd Query: Q10 */
SELECT c_custkey, c_name, l_extendedprice, l_discount, c_acctbal, n_name, c_address, c_phone, c_comment FROM CUSTOMER, ORDERS, LINEITEM, NATION WHERE C_CUSTKEY = O_CUSTKEY AND L_ORDERKEY = O_ORDERKEY AND O_ORDERDATE>= '1993-08-01' AND O_ORDERDATE < '1994-02-01' AND L_RETURNFLAG = 'R' AND C_NATIONKEY = N_NATIONKEY;
/******************/
...
/* ssd Query: Q12 */
select l_shipmode, o_orderpriority from ORDERS, LINEITEM where o_orderkey = l_orderkey and l_shipmode in ('RAIL', 'FOB') and l_commitdate < l_receiptdate and l_shipdate < l_commitdate and l_receiptdate >= '1997-01-01' and l_receiptdate < '1998-01-01';
/*****************/
...
/* ssd Query: Q14 */
select p_type, l_extendedprice, l_discount from LINEITEM, PART where l_partkey = p_partkey and l_shipdate >= '1996-12-01' and l_shipdate < '1997-01-01';
/********************/
...
/* ssd Query: Q16 */
select p_brand, p_type, p_size, ps_suppkey from PARTSUPP, PART where p_partkey = ps_partkey and p_brand <> 'Brand#34' and p_type not like 'LARGE BRUSHED%' and p_size in (48, 19, 12, 4, 41, 7, 21, 39) and ps_suppkey not in (select s_suppkey from SUPPLIER where s_comment like '%Customer%Complaints%');
/********************/

/* TPC-H Query: Q17 */
/* Doesnt complete: 6709.514s */
...
/* ssd Query: Q18 */
select c_name, c_custkey, o_orderkey, o_orderdate, o_totalprice, l_quantity from CUSTOMER, ORDERS, LINEITEM where o_orderkey in (select l_orderkey from LINEITEM group by l_orderkey having sum(l_quantity) > 314) and c_custkey = o_custkey and o_orderkey = l_orderkey;
/********************/
...
/* Host Query: Q22 */
select cntrycode, count(*) as numcust, sum(c_acctbal) as totacctbal from TABLE1 group by cntrycode order by cntrycode;

/* ssd Query: Q22 */
select substr(c_phone, 1, 2) as cntrycode, c_acctbal from CUSTOMER where substr(c_phone, 1, 2) in ('20', '40', '22', '30', '39', '42', '21') and c_acctbal > ( select avg(c_acctbal) from CUSTOMER where c_acctbal > 0.00 and substr(c_phone, 1, 2) in ('20', '40', '22', '30', '39', '42', '21')) and not exists ( select*from ORDERS where o_custkey = c_custkey);
