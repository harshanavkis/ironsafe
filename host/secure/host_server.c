...
void *consumer_func(void *args)
{
  ...
  static const unsigned long Q_MASK = BUF_POOL_SIZE - 1;
	...
	{
		void *pC = NULL;
		record_batch *ssd_record_batch;

    while(pc_state.head == pc_state.tail){}; 
    ssd_record_batch = pc_state.record_pool[pc_state.head & Q_MASK];
...
int main(int argc, char  **argv)
{
  ...
  while(nbuffer < 4096)
  {
    ...
    len = SSL_write(hSSL, temp_sub_query + nbuffer, 4096 - nbuffer);
    nbuffer += len;
    len = 0;
  }
  // free(temp_sub_query);
  ...
  while(nbuffer <  4096)
  {
    len = SSL_read(hSSL, table_schema+nbuffer, 4096-nbuffer);
    nbuffer += len;
  }
